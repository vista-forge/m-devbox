...
tag is `m-devbox:0.3.0-local` — see *For maintainers* below.)
...
| Archive | Managed by |
|---|---|
| `m-devbox_0.3.0-local.tar.zst` (amd64) | the org archive script — change-detected, refreshed by `make archive` |
| `m-devbox_0.3.0-local-arm64.tar.zst` | **manually**, saved from the remote build host |
...
```bash
docker inspect --format '{{json .Config.Labels}}' rafaelrichards/m-devbox:0.3.0
...
user. `make publish PUBLISH_TAG=0.3.0` sets both.
